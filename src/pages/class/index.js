...
export default function() {
        ...
        {set(
          ...
          (valueSet) => (
            ...
            <div className="box-w1 f_wrap" >
              ...
              <QRCode size={120} value={`http://www.aboluo.co/h5/class?product_uuid=${shareModal?.data?.product_uuid}&type=class`} id='qrCode' />
  ...
  const ShareModal = () => {
        ...
        <Form {...col} >
          {({set, form, submit, setByName, getByName}) => {
            ...
            <Forms.Item label="H5链接" >
              <Button.Group>
                <Btn type="default" width={280} disabled className="ellipsisBtn" >{`http://www.aboluo.co/h5/class?product_uuid=${shareModal?.data?.product_uuid}&type=class` || ""}</Btn>
                <CopyToClipboard text={`http://www.aboluo.co/h5/class?product_uuid=${shareModal?.data?.product_uuid}&type=class` || ""}  onCopy={(val) => $.msg('复制成功')}>
	...
	let columns = [
    {
      ...
      width: 220,
      render: (text, record) => {
        return <a className='link' target="_blank" href={`/adminPc/classInfo?product_uuid=${record.product_uuid}`} >{text}</a>
      }
		},
		{
      title: "更新进度",
      ...
      sorter: true,
		},
		{
			...
			render: (text, record) => {
        return (<div>
          <a className='link' target="_blank" href={`/adminPc/classInfo?product_uuid=${record.product_uuid}`} >详情</a>
          ...
          <Popconfirms title={record.onsell === "YES" ? "下架会同时关闭当前商品中未支付的订单信息" : "确定上架吗？"} content={record.onsell === "YES" ? "下架" : "上架"} onConfirm={async () => {
            if ((!record.cnt_lesson_finished || record.cnt_lesson_finished == 0) && record.onsell === "NO") {
              $.warning('请完善课节内容!')
              return
            }
	...
	return (
      ...
      >
        {({set, form, submit, setByName, getByName}) => (
          <div>
            <Inputs placeholder="输入课程名称搜索" className="mr_15" form={form} name="product_name" />
            ...
            <Btn onClick={() => add.open("创建课程", {}, {width: 800})} className="fl_r" >创建课程</Btn>
          </div>
        )}
      </Form>
      <TablePagination className='mt_10' api="/product/course/query" columns={columns} ref={(rs) => tableList = rs} 
        onSorter={(sort)=>{
          if (sort.order) {
            let params = {
              sort_field: 'sort',
