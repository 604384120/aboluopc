...
import TextbookAdd from "./TextbookAdd";
...
export default function() {
        ...
        {set(
          ...
          (valueSet) => (
            ...
            <div className="box-w1 f_wrap" >
              ...
              <QRCode size={120} value={`http://www.aboluo.co/h5/class?product_uuid=${shareModal?.data?.product_uuid}&type=material`} id='qrCode' />
  ...
  const ShareModal = () => {
          ...
          {({set, form, submit, setByName, getByName}) => (
            <div>
              <Forms.Item label="H5链接" >
                <Button.Group>
                  <Btn type="default" width={280} disabled className="ellipsisBtn" >{`http://www.aboluo.co/h5/class?product_uuid=${shareModal?.data?.product_uuid}&type=material` || ""}</Btn>
                  <CopyToClipboard text={`http://www.aboluo.co/h5/class?product_uuid=${shareModal?.data?.product_uuid}&type=material` || ""}  onCopy={(val) => console.log(val)}>
	...
	let columns = [
    {
      ...
      width: 220,
      render: (text, record) => {
        return <a className='link' target="_blank" href={`/adminPc/TextbookInfo?product_uuid=${record.product_uuid}`} >{text}</a>
      }
		},
		{
      title: "总页数",
      ...
      sorter: true,
		},
		{
			...
			render: (text, record) => {
        return (<div>
          <a className='link' target="_blank" href={`/adminPc/TextbookInfo?product_uuid=${record.product_uuid}`} >详情</a>
          ...
          <Popconfirms title={record.onsell === "YES" ? "下架会同时关闭当前商品中未支付的订单信息" : "确定上架吗？"} content={record.onsell === "YES" ? "下架" : "上架"} onConfirm={async () => {
            let res = await $.post('/product/material/update', {onsell: record.onsell === "YES" ? 'NO' : "YES", product_uuid: record.product_uuid});
            tableList.reload();
          }} />
          <Divider type="vertical" />
          <Popconfirms title={record.show === "YES" ? "隐藏后，商品不展示在APP端，仅可通过分享链接进行查看" : "确定展示吗？"} content={record.show === "YES" ? "隐藏" : "展示"} onConfirm={async () => {
            let res = await $.post('/product/material/update', {show: record.show === "YES" ? 'NO' : "YES", product_uuid: record.product_uuid});
            tableList.reload();
          }} />
          <Divider type="vertical" />
      ...
      >
        {({set, form, submit, setByName, getByName}) => (
          <div>
            ...
            <Btn onClick={() => add.open("创建教材", {}, {width: 800})} className="fl_r" >创建教材</Btn>
          </div>
        )}
      </Form>
      <TablePagination className='mt_10' api="/product/material/query" columns={columns} ref={(rs) => tableList = rs} 
        onSorter={(sort)=>{
          if (sort.order) {
            let params = {
              sort_field: 'sort',
