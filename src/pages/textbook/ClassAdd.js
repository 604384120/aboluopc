  ...
  const AddMedia = ({ form, set, getByName }) => {
    ...
    return (
      <Forms.Item label="课程封面" required={true} >
        {set(
          {name: 'product_video', value: infoData?.product_video, required: true},
          (valueSet) => (
      ...
      >
        {({set, form, submit, setByName, getByName}) => (
          <div>
            ...
            <Forms.Item label="课程排序" >
              <Inputs form={form} name="sort" value={infoData?.sort} width={260} type="inputNumber" placeholder="数字越小越靠前" />
            </Forms.Item>
            ...
            <Forms.Item label="售卖时间" required={true} >
              <Inputs name="start_time" form={form} type="dateTimePicker" placeholder="开始时间" className="mr_15" required={true} showTime={{ format: 'HH:mm' }} format={"YYYY-MM-DD HH:mm"} value={infoData?.start_time} />
              <Inputs name="end_time" form={form} type="dateTimePicker" placeholder="结束时间" showTime={{ format: 'HH:mm' }} required={true} format={"YYYY-MM-DD HH:mm"} value={infoData?.end_time} />
            </Forms.Item>
            <Forms.Item label="展示/隐藏" >
              <Inputs form={form} name="show" value={infoData?.show || "YES"} radios={[
                {text: '展示', value: 'YES'},
                {text: '隐藏', value: 'NO'},
              ]} width={260} />
            </Forms.Item>
            <Forms.Item label="上/下架" >
