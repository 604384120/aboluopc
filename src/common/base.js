...
import { Layout, Menu, Icon, Skeleton, Progress, Modal, Breadcrumb } from "antd";
import $$ from "jquery";
...
export class SuspenseSwitchRoute extends React.PureComponent {
	...
	routeWithSubRoutes(props) {
		...
		useEffect(() => {
			this.$.store("BCB_setBarName", name);
			this.$.store("BCB_setBarPath", path);
	...
	render() {
		...
		// $store("routesPathList", route.path);
		...
		return (
			<Suspense fallback={<Skeleton paragraph={{ rows: 10 }} active />}>
				<Switch>
					...
					/>
					{$routes.map((route, i) => {
						return <RouteWithSubRoutes store={$store} key={i} {...route} />
					})}
...
export class SiderMenuTop extends React.PureComponent {
	...
	async getUser() {
		...
		params.orgtype = GlobalIdentity.toUpperCase();  // 所有接口共同带的参数，这个参数是个标识，用来标识是小程序还是PC
				...
				>
					...
					>
						<img
							...
							src="https://sxzimgs.oss-cn-shanghai.aliyuncs.com/yingxiao/page/09caa1e2-4f48-11eb-a82b-00163e04cc20.png"
						/>
					</div>
					{routes.map((route, i) => {
						if (route.hide === true) {
